...
<template>
  <section v-if="step.plan?.length" class="timeline-activity plan-activity">
    <header><ListChecks :size="15" /><b>{{ t('timeline.details.plan') }}</b><span>{{ completedPlans }}/{{ step.plan.length }}</span></header>
    <ol>
  ...
  <section v-if="!hideEvidence && step.tests?.length" class="timeline-activity">
    <header><Beaker :size="15" /><b>{{ t('timeline.details.tests') }}</b></header>
  ...
  <section v-if="!hideEvidence && step.changes?.length" class="timeline-activity">
    <header><FileDiff :size="15" /><b>{{ t('timeline.details.changes') }}</b></header>
  ...
  <section v-if="step.subagents?.length" class="timeline-activity">
    <header><Bot :size="15" /><b>{{ t('timeline.details.agents') }}</b></header>
  ...
  <section v-if="step.workflowTasks?.length" class="timeline-activity workflow-activity">
    <header><GitBranch :size="15" /><b>{{ t('timeline.details.workflowTasks') }}</b><span>{{ completedWorkflowTasks }}/{{ step.workflowTasks.length }}</span></header>
  ...
  <section v-if="step.workflowHandoffs?.length" class="timeline-activity workflow-activity">
    <header><ArrowRightLeft :size="15" /><b>{{ t('timeline.details.handoffs') }}</b><span>{{ step.workflowHandoffs.length }}</span></header>
  ...
  <section v-if="step.workflowReviews?.length" class="timeline-activity workflow-activity">
    <header><ShieldCheck :size="15" /><b>{{ t('timeline.details.reviews') }}</b></header>
  ...
  <section v-if="step.skills?.length" class="timeline-activity">
    <header><Sparkles :size="15" /><b>{{ t('timeline.details.skills') }}</b></header>
  ...
  <section v-if="step.logs?.length" class="timeline-activity logs-activity">
    <button type="button" @click="logsOpen = !logsOpen">
      <ScrollText :size="15" /><b>{{ t('timeline.details.logs') }}</b><span>{{ step.logs.length }}</span><ChevronDown :size="14" />
    </button>
