...
export const SztubuddyRoot = path.join(runtimeRoot, "prompts", "Sztubuddy");
...
export function SztubuddyManifest(): Manifest {
  if (!manifest) {
    const value = JSON.parse(fs.readFileSync(path.join(SztubuddyRoot, "manifest.json"), "utf8")) as Manifest;
    if (value.version !== 1 || !Array.isArray(value.files) || !Array.isArray(value.skills) || !Array.isArray(value.agents)) throw new Error("Invalid Sztubuddy resource manifest");
    manifest = value;
  }
  return manifest;
}
...
export function resourcePath(relative: string): string {
  ...
  const candidate = path.resolve(SztubuddyRoot, relative);
  const rel = path.relative(SztubuddyRoot, candidate);
  if (!rel || rel.startsWith("..") || path.isAbsolute(rel)) throw new Error("Resource path escapes the Sztubuddy bundle");
  const real = fs.realpathSync(candidate);
  const realRel = path.relative(fs.realpathSync(SztubuddyRoot), real);
  if (realRel.startsWith("..") || path.isAbsolute(realRel)) throw new Error("Resource symlink escapes the Sztubuddy bundle");
  return real;
}

export function adaptSztubuddyText(text: string): string {
  return text.replace(/({{[\s\S]*?}}|{%[\s\S]*?%})/g, expr => expr.replace(/===/g, "==").replace(/==\s*undefined/g, "is undefined").replace(/([A-Za-z_][\w.]*)\.length/g, "($1 | length)").replace(/([A-Za-z_][\w.]*)\.join\(([^)]*)\)/g, "($1 | join($2))"))
    .replace(/\b(CodeBuddy Code|CodeBuddy|Sztubuddy)\b/g, "SztuCode")
...
export function renderSztubuddyText(text: string, variables: Record<string, unknown> = {}): string {
  const source = adaptSztubuddyText(text);
...
export function loadSztubuddyResource(relative: string, variables: Record<string, unknown> = {}): string {
  return renderSztubuddyText(fs.readFileSync(resourcePath(relative), "utf8"), variables);
}

export function SztubuddyContract(): string { return fs.readFileSync(resourcePath("runtime-contract.md"), "utf8").trim(); }

export function SztubuddySkills(): ImportedSkill[] { return [...SztubuddyManifest().skills, ...SztubuddyManifest().commands]; }

export function SztubuddyPlugins(): { name: string; path: string; skills: string[] }[] {
  const names = [...new Set(SztubuddyManifest().skills.map(skill => skill.plugin).filter((name): name is string => Boolean(name)))];
  return names.map(name => ({ name, path: resourcePath(`skills/_builtin-plugins/${name}`), skills: SztubuddyManifest().skills.filter(skill => skill.plugin === name).map(skill => skill.name) }));
}

export function buildSztubuddyBase(): string {
  // Keep the initial injection small. Product workflows and skills are loaded on demand.
  return [loadSztubuddyResource("main/sztucode-core.tpl"), SztubuddyContract()].filter(Boolean).join("\n\n");
}
export function SztubuddyMode(mode: InteractionMode): string {
  if (!["ask", "craft", "plan", "expert"].includes(mode)) throw new Error(`Unknown interaction mode: ${mode}`);
  const files = SztubuddyManifest().files.filter(file => file.path.startsWith(`modes/${mode}/fragments/`) && !file.path.endsWith("interaction.md"));
  return [...files.map(file => loadSztubuddyResource(file.path)), ...(mode === "plan" ? ["Plan mode is active. Only inspect and plan. Do not modify project files or execute commands until the runtime exits plan mode."] : []), SztubuddyContract()].filter(Boolean).join("\n\n");
}

export function importedAgent(name: string): ImportedAgent | undefined {
  const agent = SztubuddyManifest().agents.find(agent => agent.name === name);
...
export function importedToolDescription(name: string): string | undefined {
  const upstream = SztubuddyManifest().toolMapping[name === "subagent_result" ? "agent_result" : name];
  if (!upstream) return undefined;
  return loadSztubuddyResource(`product/tool-${upstream}-description.tpl`) + "\n\n" + SztubuddyContract();
}
...
export function readPromptResource(relative = "", offset = 0, limit = 200): string {
  ...
  if (!relative || relative.endsWith("/")) {
    const files = SztubuddyManifest().files.filter(file => file.path.startsWith(relative)).map(file => file.path);
    return JSON.stringify({ path: relative, total_files: files.length, files: files.slice(offset, offset + limit), next_offset: offset + limit < files.length ? offset + limit : null, limitations: SztubuddyManifest().limitations });
  }
  const content = relative.startsWith("skills/") ? adaptSztubuddyText(fs.readFileSync(resourcePath(relative), "utf8")) : loadSztubuddyResource(relative);
