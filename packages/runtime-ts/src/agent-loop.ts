...
export class AgentLoop {
  ...
  async run(runId: string, goal: string, maxSteps = 100, history: ChatMessage[] = [], signal?: AbortSignal, takeSteering?: () => ChatMessage[], steeringSignal?: () => AbortSignal, userContent: ChatMessage["content"] = goal): Promise<AgentRunResult> {
    ...
    const context = new ContextManager([...history, { role: "user", content: userContent }], { maxTokens: resolveContextWindow(this.options.contextWindow), reservedOutputTokens: this.options.maxOutputTokens ?? 8_192, maxToolResultChars: 8_000 }, TokenCounter.forModel(this.options.provider, this.options.model));
