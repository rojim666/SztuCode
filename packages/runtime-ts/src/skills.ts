...
export class SkillLoader {
  ...
  async list(): Promise<Skill[]> {
    ...
    for (const [root, source, scope] of roots) { try { for (const entry of await fsPromises.readdir(root, { withFileTypes: true })) { const candidate = entry.isDirectory() ? path.join(root, entry.name, "SKILL.md") : entry.name.toLowerCase().endsWith(".md") ? path.join(root, entry.name) : ""; if (!candidate) continue; const header = await readFrontmatterHeader(candidate); if (header === null) continue; /* 无 frontmatter 或解析失败：跳过且不抛错 */ const { values, tools } = parseHeader(header); const name = values.get("name") || path.basename(path.dirname(candidate)); const id = `${source}:${name}`; const plugin = source.includes(":") ? source.slice(source.indexOf(":") + 1) : null; seen.set(name, { id, name, display_name: name, description: values.get("description") ?? "", short_description: values.get("description") ?? "", source, scope, path: candidate, enabled: (enabled[id] ?? true) && (pluginRoots.find((item) => item.source === source)?.enabled ?? true), get system_prompt_template(): string { return lazyBody(candidate); }, allowed_tools: tools, plugin, icon: null, brand_color: null, allow_implicit_invocation: true }); } } catch { /* optional root */ } }
    return [...seen.values()];
  }
