...
export class ServerService {
  ...
  async dispatch(this: any, request: JsonRpcRequest, socket: net.Socket): Promise<JsonRpcResponse> {
    switch (request.method) {
      ...
      case "artifact.restore": { const p = request.params as { workspace_id: string; artifact_id: string; version: number; expected_hash: string }; if (!Number.isSafeInteger(p.version) || p.version < 1 || !/^[a-f0-9]{64}$/.test(p.expected_hash ?? "")) throw new Error("version and expected_hash are required"); const workspace = await this.workspaces.get(p.workspace_id); return ok(request.id, { artifact: await this.artifacts.restore(p.workspace_id, workspace.path, p.artifact_id, p.version, p.expected_hash) }); }
      case "artifact.register":
      case "artifact.create": { const p = request.params as { workspace_id: string; path: string; artifact_type?: any; summary?: string; session_id?: string; run_id?: string; input_sources?: any[] }; const workspace = await this.workspaces.get(p.workspace_id); const artifact = await this.artifacts.register(p.workspace_id, workspace.path, p.path, { ...(p.artifact_type ? { type: p.artifact_type } : {}), summary: p.summary ?? "", session_id: p.session_id, run_id: p.run_id, input_sources: p.input_sources ?? [] }); this.events.publish({ type: "log.line", run_id: p.run_id ?? "artifact", level: "INFO", source: "artifact", message: `artifact ${artifact.artifact_id} registered`, ts: new Date().toISOString() }); return ok(request.id, { artifact }); }
      ...
      case "skill.uninstall": { const params = request.params as { skill_id: string; workspace_id?: string | null; confirm?: string }; if (params.confirm !== "uninstall") throw new Error("confirm=uninstall is required"); const root = params.workspace_id ? (await this.workspaces.get(params.workspace_id)).path : process.cwd(); await new SkillLoader(root).uninstall(params.skill_id); return ok(request.id, { skill_id: params.skill_id, uninstalled: true }); }
