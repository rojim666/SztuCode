...
export class OpenAiCompatibleProvider implements ModelProvider {
  ...
  async complete(messages: ChatMessage[], tools: ToolRegistry, signal?: AbortSignal, onToken?: (token: string) => void, _invocation?: ModelInvocation, _onThinking?: (thinking: string) => void): Promise<ModelResponse> {
    ...
    try {
      ...
      const body = responses ? { model: this.options.model, ...(system ? { instructions: system } : {}), input, tools: definitions, max_output_tokens: this.options.maxOutputTokens, ...(this.options.stream ? { stream: true } : {}), ...(this.options.temperature != null ? { temperature: this.options.temperature } : {}), ...(this.options.topP != null ? { top_p: this.options.topP } : {}), ...(this.options.reasoningEffort ? { reasoning: { effort: this.options.reasoningEffort } } : {}) } : { model: this.options.model, messages: apiMessages, tools: definitions.map((tool) => ({ type: "function", function: { name: tool.name, description: tool.description, parameters: tool.parameters }, ...(tool.cache_control ? { cache_control: tool.cache_control } : {}) })), tool_choice: "auto", ...(this.options.stream ? { stream: true, stream_options: { include_usage: true } } : {}), ...(this.options.maxOutputTokens ? { max_tokens: this.options.maxOutputTokens } : {}), ...(this.options.temperature != null ? { temperature: this.options.temperature } : {}), ...(this.options.topP != null ? { top_p: this.options.topP } : {}) };
      ...
      if (this.options.stream && response.body && response.headers.get("content-type")?.includes("text/event-stream")) return await this.parseStream(response, responses, onToken);
      const payload = await response.json() as OpenAiResponse & ResponsesResponse;
      if (responses) return this.parseResponses(payload);
  ...
  private parseResponses(payload: ResponsesResponse): ModelResponse {
    ...
    return { text, tool_calls, stop_reason: tool_calls.length ? "tool_use" : "end_turn", model: this.options.model, usage: { input_tokens: Number(payload.usage?.input_tokens ?? 0), output_tokens: Number(payload.usage?.output_tokens ?? 0), cache_read_input_tokens: Number(payload.usage?.input_tokens_details?.cached_tokens ?? 0) } };
  }
}
