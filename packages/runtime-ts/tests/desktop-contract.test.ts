...
test("Tauri production bundles and starts the TypeScript runtime resource", async () => {
  ...
  const pdfParser = await readFile(path.join(repositoryRoot, "packages/runtime-ts/src/document-parser/pdf.ts"), "utf8");
  ...
  assert.match(prepare, /runtime-ts/); assert.match(prepare, /process\.execPath/); assert.match(prepare, /esbuildIsScript/); assert.match(prepare, /chmod\(bundledNode, 0o755\)/); assert.match(prepare, /resources["']?,\s*["']runtime/); assert.match(prepare, /prepareSkillAssets/); assert.match(prepare, /\["prompts",\s*"agents"\]/); assert.match(prepare, /@napi-rs/); assert.match(prepare, /canvas-/);
  assert.match(pdfParser, /await import\(["']pdf-parse["']\)/, "PDF dependencies must initialize lazily so an optional native module cannot crash daemon startup");
  assert.doesNotMatch(pdfParser, /^import\s+\{\s*PDFParse\s*\}\s+from\s+["']pdf-parse["']/m);
});
