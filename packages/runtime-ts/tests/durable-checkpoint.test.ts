...
test("run manager durably checkpoints model context after tool batches and completion", async () => {
  ...
  try {
    ...
    const lifecycle: string[] = []; const finished = new Promise<void>((resolve) => events.subscribe((event) => { if (event.type === "operation.started" || event.type === "operation.finished") lifecycle.push(event.type); if (event.type === "run.finished") resolve(); })); const runId = manager.start("read it", [], undefined, root, session.id); await sessions.attachRun(session.id, runId); await finished;
    assert.deepEqual(lifecycle, ["operation.started", "operation.finished"]);
    ...
    assert.deepEqual(checkpoints.map((record) => record.phase), ["completed"]);
    assert.deepEqual(checkpoints.map((record) => record.sequence), [2]);
    assert.ok(checkpoints.every((record) => record.run_id === runId && record.operation_id === runId));
    assert.deepEqual(checkpoints.map((record) => record.checkpoint_id), [`${runId}:2`]);
