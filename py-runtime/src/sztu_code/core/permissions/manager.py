...
class PermissionManager:
    ...
    def on_mode_change(
        self,
        listener: Callable[[PermissionMode, PermissionMode], Awaitable[None]],
    ) -> None:
    ...
    def cancel_run(self, run_id: str, session_id: str) -> None:
        to_cancel = [
            uid
            for uid, req in self._pending.items()
        ...
        for uid in to_cancel:
            ...
            if not req.future.done():
                logger.debug(
                    "permission: cancel pending for run tool_use_id=%s reason=run_cancelled",
                    uid,
                )
                req.future.set_result("deny_once")
