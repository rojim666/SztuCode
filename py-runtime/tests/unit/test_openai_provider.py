...
# 功能：验证预算准入收缩的单次输出上限被写入请求参数 max_completion_tokens（Issue #72）
# 设计：默认调用断言使用构造配置的 8192；显式 max_output_tokens 调用断言覆盖生效
async def test_chat_max_output_tokens_override() -> None:
    chunks = [
        _make_chunk(content="hi"),
        _make_chunk(finish_reason="stop", usage=_make_usage()),
    ]
    provider, client = _make_provider(chunks)
    await _chat(provider)
    assert client.chat.completions.create.call_args.kwargs["max_completion_tokens"] == 8192

    chunks2 = [
        _make_chunk(content="hi"),
        _make_chunk(finish_reason="stop", usage=_make_usage()),
    ]
    provider2, client2 = _make_provider(chunks2)
    await provider2.chat(
        messages=[], tool_schemas=[], bus=EventBus(), run_id="r1",
        max_output_tokens=123,
    )
    assert client2.chat.completions.create.call_args.kwargs["max_completion_tokens"] == 123
