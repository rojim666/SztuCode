...
SztuCode 是本地优先的 TypeScript daemon/client Agent 系统。客户端通过 TCP 上的 NDJSON/JSON-RPC 2.0 连接，Agent 执行状态由唯一的 TypeScript runtime 管理。
...
历史 Python runtime 保留在 `python-runtime` 分支，不属于当前产品路径。
